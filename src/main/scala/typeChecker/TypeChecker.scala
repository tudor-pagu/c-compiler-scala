...
object TypeCheck {
  def typeOf(e: AstExt, nv: TypeEnvironment): Type = e.node match {
    ...
    case AstExtKind.Binary(op, l, r) => (op, typeOf(l, nv), typeOf(r, nv)) match {
      ...
      case _ => throw createError(s"Invalid binary operation: ${op.toString} between ${typeOf(l, nv).toString} and ${typeOf(r, nv).toString}", e.span)
    }
    case AstExtKind.Identifier(name) => lookupType(e, nv)
    case AstExtKind.PrefixOperation(op, e) => (op, typeOf(e, nv)) match {
      case (PrefixOp.Negation, t@NumT(size, signed)) => Type.numericalPromotion(t)
      case (PrefixOp.UnaryPlus, t@NumT(size, signed)) => Type.numericalPromotion(t)
      case (_,t) => throw createError(s"Invalid unary opration on ${t.toString}", e.span)
    }
      case AstExtKind.PostfixOperation(op, e) => (op, typeOf(e, nv)) match {
        case (PostfixOp.FunctionCall(args), t@FunT(returnType, params)) => {
          if (args.size != params.size) {
            throw createError(s"Tried to call a function with {args.size} arguments, when the function has ${params.size} parameters", e.span)
          }
          val argTypes = args.map(typeOf(_, nv))
          if (argTypes != params) {
            throw createError(s"Mismatch between types expected by function and arguments.", e.span)
          }
          returnType
        }
